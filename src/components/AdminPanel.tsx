...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [newAdminPasswordInput, setNewAdminPasswordInput] = useState<string>(adminPassword);
  const [adminPassSaveSuccess, setAdminPassSaveSuccess] = useState<boolean>(false);
  ...
  const handleSaveAdminPassword = () => {
    ...
    setAdminPassSaveSuccess(true);
    setTimeout(() => setAdminPassSaveSuccess(false), 4000);
  };
    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs z-50 overflow-y-auto p-3 sm:p-6 flex items-center justify-center">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-5xl my-auto overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {/* TAB 4: CLASS-WISE OTPS & SECURITY */}
          {activeTab === 'otps' && (
            <div className="space-y-6">
              {/* ADMIN PASSWORD CHANGE CARD */}
              <div className="bg-white border border-red-200 p-6 rounded-xl space-y-4 shadow-2xs">
                <div className="flex items-center gap-2 border-b border-red-100 pb-3">
                  <div className="p-2 bg-red-100 text-red-700 rounded-lg">
                    <Shield className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-base font-black text-gray-900 uppercase">Admin Panel Access Password</h3>
                    <p className="text-xs text-gray-500 font-medium">
                      Change the master password required to log in to this FCPS Admin Control Panel.
                    </p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2.5 text-xs bg-gray-50 border border-gray-300 rounded-lg font-bold focus:bg-white focus:outline-none focus:border-red-600"
                      />
                    </div>
                    <span className="text-[10px] text-gray-400 block pt-1">
                      Current Password: <strong className="text-gray-700">{adminPassword}</strong>
                    </span>
                  </div>

                  <button
                    onClick={handleSaveAdminPassword}
                    className="px-5 py-2.5 bg-red-700 hover:bg-red-800 text-white font-bold text-xs uppercase tracking-wider rounded-lg flex items-center justify-center gap-2 cursor-pointer shadow-sm transition-colors"
                  >
                    <Save className="w-4 h-4" />
                    <span>Update Admin Password</span>
                  </button>

                  {adminPassSaveSuccess && (
                    <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold rounded-lg flex items-center gap-2">
                      ...
                      <span>Admin Password updated and synchronized across all devices successfully!</span>
                    </div>
                  )}
                </div>
              </div>
