...
import { Download, FileText, Image as ImageIcon, RotateCcw, CheckCircle, XCircle, MinusCircle, Award, Printer } from 'lucide-react';
...
export const ResultCard: React.FC<ResultCardProps> = ({ results, onRetakeTest, onGoHome }) => {
  ...
  // Helper function to temporarily remove max-height & overflow restrictions for complete canvas capture
  const prepareContainerForCapture = () => {
    if (!exportRef.current) return null;
    const scrollContainer = exportRef.current.querySelector('.overflow-y-auto') as HTMLElement;
  ...
  const handleExportImage = async () => {
    ...
    const restoreStyles = prepareContainerForCapture();
    ...
    try {
      ...
      alert('Generating image failed. You can use the "Print / Save PDF" button as a direct fallback.');
    } finally {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      alert('Generating PDF failed. You can use the "Print / Save PDF" button as a direct fallback.');
    } finally {
      if (restoreStyles) restoreStyles();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Top Action Buttons */}
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-2xs no-print">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-md font-bold text-xs uppercase tracking-wider transition-colors cursor-pointer shadow-2xs"
          >
      ...
      <div
        ref={exportRef}
        className="print-area bg-white rounded-xl border border-slate-200 p-6 sm:p-8 shadow-xs space-y-6 text-slate-900"
      >
        {/* Header Branding */}
